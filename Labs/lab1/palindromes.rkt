...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (and (all-palindromes? n (cdr Bases)) (palindrome? (num->base n (car Bases))))))
...
(define (palindromes-to-n n Bases)
  (if (zero? n)
      (list 0)
      (if (all-palindromes? n Bases) (append (palindromes-to-n (sub1 n) Bases) (list n)) (append (palindromes-to-n (sub1 n) Bases) null))))
...
(define (multiple_palindrom? n b base)
  (if (> base 10)
      (<= b 0)
      (if (palindrome? (num->base n base))
          (multiple_palindrom? n (sub1 b) (add1 base))
          (multiple_palindrom? n b (add1 base)))))

(define (first-b-pal start b)
  (if (multiple_palindrom? start b 2)
      start
...
(define (longest-palindrome n)
  (if (or (< n 10) (palindrome? (num->base n 10)))
      n
      (max (longest-palindrome (quotient n 10)) (longest-palindrome (list->num (cdr (num->base n 10)))))))
