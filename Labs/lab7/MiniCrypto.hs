...
{-
  ...
  modulului Data.List.
  http://hackage.haskell.org/package/talbase-4.6.0.1/docs/Data-List.html
...
arithmetic first ratio = iterate (+ ratio) first
geometric :: Num a => a -> a -> [a]
geometric first ratio = iterate (* ratio) first 
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = map (\x -> fromIntegral $ fst $ x) (iterate (\x -> next $ snd $ x) (next $ mkStdGen seed))
{-
    4. (1p)
...
tableToFunc table char = let encriptare = head [snd x | x <- table, (fst x) == char] 
                         in encriptare
