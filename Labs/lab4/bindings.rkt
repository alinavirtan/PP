...
(define (distance p1 p2)
  (let* ((segment (list p1 p2))
         (get-segm (λ () segment))
         (get-start (λ (segment) (car segment)))
         (get-end (λ (segment) (cadr segment))))
...
(define (compute-perimeter points)
  (let iter ((first-point (first points))
              (last-point (last points))
              (cur-points points))
    (if (null? (cdr cur-points))
        (distance first-point last-point)
        (+ (distance (car cur-points) (cadr cur-points)) (iter first-point last-point (cdr cur-points)))
    )))
...
(define (3-sequence-max numbers separator)
  (let*-values (((sep?) (λ (x) (if (= x separator) #f #t)))
                ((secv1-p1 secv1-p2) (splitf-at numbers sep?))
                ((secv2-p1 secv2-p2) (splitf-at (cdr secv1-p2) sep?))
                ((secv3-p1 secv3-p2) (splitf-at (cdr secv2-p2) sep?)))
      (max (max (sum secv1-p1) (sum secv2-p1)) (sum secv3-p1)) 
  ))
...
(define (list-num-concat numbers)
    (apply string-append (map number->string numbers)))
...
(define (find-all-suffixes number)
  (let iter ((lst (number->list number))
              (result '()))
       (if (null? lst)
           result
           (iter (cdr lst) (append result (list (string->number (list-num-concat lst))))))))
...
(define (run initial-state final-state next)
  (let iter ((init initial-state)
             (result null))
    (if (= init final-state)
        (append result (list final-state)) 
        (iter (next init) (append result (list init))))))
...
(define (generate-number k x)
  (string->number (list-num-concat (run x (+ (* (sub1 k) k) x) (lambda (y) (+ y k))))))
