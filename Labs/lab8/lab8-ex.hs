...
lengthV (V a1 a2 a3) = sqrt $ a1 * a1 + a2 * a2 + a3 * a3
   
normalizeV :: Vector -> Vector
normalizeV (V a1 a2 a3) = V (a1/norma) (a2/norma) (a3/norma)
  where
    norma = lengthV (V a1 a2 a3)
...
orthogonalV (_ : []) = True
orthogonalV (x : xs) = if (orto x xs) == True then orthogonalV xs else False
  where
    orto x [] = True
    orto x (y : xs) = if (dotV x y) == 0 then (orto x xs) else False
...
data SList a = SNil | SCons a (SList a) | SConsList (SList a) (SList a) deriving Show

emptySList :: SList a
emptySList = SNil

consElem :: a -> SList a -> SList a
consElem x lst = SCons x lst

consList :: SList a -> SList a -> SList a
consList lst lista = SConsList lst lista  

headSList :: SList a -> SList a
headSList (SCons x xs) = SCons x SNil
headSList (SConsList xs xy) = xs

tailSList :: SList a -> SList a
tailSList (SCons x xs) = xs
tailSList (SConsList xs ys) = ys

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual SNil SNil = True
deepEqual (SCons x xs) (SCons y ys) = if (x == y) then (deepEqual xs ys) else False
deepEqual (SConsList xs1 xs2) (SConsList ys1 ys2) = if (deepEqual xs1 ys1) == True then (deepEqual xs2 ys2) else False
deepEqual _ _ = False

flatten :: SList a -> [a]
flatten (SCons x SNil) = [x]
flatten (SCons x y) = [x] ++ (flatten y)
flatten (SConsList xs ys) = (flatten xs) ++ (flatten ys)
...
data Lst a b = LNil | Consa a (Lst a b) | Consb b (Lst a b)
...
insertElem BSTNil x = TreeNode x BSTNil BSTNil  
insertElem (TreeNode val left right) x = if x < val 
                                            then (TreeNode val next_left right) 
                                            else (TreeNode val left next_right) 
                                    where next_left = insertElem left x
                                          next_right = insertElem right x

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem (TreeNode val left right) x = if (val == x) 
                                          then Just x 
                                          else if (x <= val) 
                                                  then findElem left x 
                                                  else findElem right x


subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (TreeNode val left right) x y 
  | (findElem (TreeNode val left right) x) /= Nothing && (findElem (TreeNode val left right) y) /= Nothing = Just (TreeNode val left right)
  | (findElem (TreeNode val left right) x) == Nothing || (findElem (TreeNode val left right) x) == Nothing = Nothing
