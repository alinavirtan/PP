...
map1 f list = foldr (\item acc -> f item:acc) [] list

map2 f list = foldl (\acc item -> f item:acc) [] list

map3 f list = [f x | x <- list]

map4 f (x:xs) = f x : map4 f xs

take 5 $ zipWith (+) (iterate (+1) 0) (iterate (+2) 0)

f x y z = if x then if (length $ head y) == (head $ tail z)
		       then "Success"
		       else "Failure"
		else "Failure"
{-
...
unzip2  :: [(a, b)] -> ([a], [b])
unzip2 [] = ([],[])
unzip2 xs = (map fst xs, map snd xs) -- face o lista in care pune primul element din fiecare lista
				     -- face o alta lista in care pune al doilea element din fiecare lista
				     -- face o pereche din cele 2 liste
...
prime 1 = False
prime 2 = True
prime n =  [x | x <- [2 .. n `quot` 2], n `mod` x == 0] == []
primes n = [x | x <- [2..n], prime x]

-- SAU
--primes 1 = []
--primes n = [i | i <- [2..n], and [i `mod` k /= 0 | k <- primes (i `quot` 2)]]
...
setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = [x | x <- (setIntersection a b ++ setDiff a b ++ setDiff b a)]

-- Verificare: check4  [3, 7] [8, 10, 4] [1, 2, 6]
check3 :: TestPP ()
check3 = do
...
group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 (x:xs) = makeGroupRecursive xs x [[x]]

makeGroupRecursive :: Eq a => [a] -> a -> [[a]] -> [[a]]
makeGroupRecursive [] prev sol = sol
makeGroupRecursive (x:xs) prev [] = makeGroupRecursive xs prev [[x]]
makeGroupRecursive (x:xs) prev sol
                | x == prev = makeGroupRecursive xs x ((init sol) ++ [x : (last sol)]) 
                | otherwise = makeGroupRecursive xs x (sol ++ [[x]])
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc xs = [(head lst, length lst) | lst <- group2 $ sort xs]
...
-}
dup :: String -> String
dup xs = unwords [(cuvant ++ " " ++ cuvant) | cuvant <- words xs]
...
isIsogram xs = [x | x <- nrOcc xs, (isLetter $ fst x) && (snd x > 1)] == []
