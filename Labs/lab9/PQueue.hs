...
data MyBool = MyFalse | Anything | MyTrue

data TList a = Nil | Cons a (TList a)
...
class (Ord a) => PQueue pq a where
    ...
    toList :: pq a -> [(Prio, a)]
    toList q = if isEmpty q then [] else (fromJust $ top q) : (toList (pop q))

    size :: pq a -> Int
    size q = length $ toList q -- Pentru Ex 6
...
{-
    ...
    listă de elemente ordonata descrescator dupa priortate. Includeți ListPQ în clasa PQueue.
-}

{-newtype ListPQ a = LPQ <trenuie inlocuit cu ceva care reprezinta o lista de tupluri> deriving Show-}

newtype ListPQ a = LPQ [(Prio, a)] deriving Show
instance (Ord a) => PQueue ListPQ a where
    empty = LPQ []

    insert pair  (LPQ []) = LPQ [pair]
    insert pair (LPQ pq) = let elemPrio = fst pair
                               firstPrio = fst $ head pq
                               beging_insert = LPQ (pair : pq)
                               headOfList = head pq
                               tailOfList = pop (LPQ pq)
                               in_between_insert = LPQ (headOfList : toList (insert pair tailOfList))
                           in if elemPrio >= firstPrio then beging_insert else in_between_insert


    top (LPQ pq) = if pq == [] then Nothing else Just (head pq)
...
merge (Empty _) (Empty _) = (Empty 0)
merge tree1 tree2 
  | fst (nodeVal tree1) < fst (nodeVal tree2) = merge tree2 tree1
  | rank_left >= rank_merged = (Node (rank_merged + 1) (nodeVal tree1) (left tree1) merged_tree)
  | otherwise = (Node (rank_left + 1) (nodeVal tree1) merged_tree (left tree1))
  where 
    merged_tree = merge (right tree1) tree2
    rank_left = rank (left tree1)
    rank_merged = rank merged_tree
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    isEmpty (Node rank nodeVal left right) = False
    
    insert nodeVal pq = merge (Node 1 nodeVal empty empty) pq

    top tree = if isEmpty tree then Nothing else Just (nodeVal tree)

    pop tree = merge (left tree) (right tree)
